...
@MainActor extension ScriptletsTabHelper: ScriptletsTabHelperBridge {
  ...
  public func requestScriptlets(frameURL: URL) async -> [String] {
    ...
    let result: [String] = models.compactMap { tuple in
      ...
      let isDeAmpEnabled = tab?.profile.prefs.boolean(forPath: kDeAmpEnabled) ?? true
      let deAmpEnabled = "const deAmpEnabled = \(isDeAmpEnabled);"
      return [scriptletGlobals, deAmpEnabled, script].joined(separator: "\n")
    }
    return result
  }
}
