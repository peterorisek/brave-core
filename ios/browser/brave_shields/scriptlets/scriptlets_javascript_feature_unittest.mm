...
class ScriptletsJavaScriptFeatureTest : public IOSChromeTestWithWebState {
 protected:
  ...
  // `frame_url` the same way the Swift `ScriptletsTabHelper` does. The optional
  // `extra_globals` is prepended after `scriptletGlobals`, mirroring how the
  // Swift helper injects the `deAmpEnabled` const.
  NSArray<NSString*>* ScriptletsForRules(
      NSString* rules,
      NSString* frame_url,
      const std::string& resources_json = SetResourcesJSON(),
      const std::string& extra_globals = std::string()) {
    ...
    return @[ base::SysUTF8ToNSString(base::StrCat(
        {scriptlet_globals, extra_globals, *injected_script})) ];
  }
...
TEST_F(ScriptletsJavaScriptFeatureTest, CheckForDeAmpPref) {
  ...
  NSArray<NSString*>* enabled_scriptlets =
      ScriptletsForRules(@"b.*##+js(deamp)", @"https://b.com", resources_json,
  ...
  web::test::LoadHtml(@"<html><body></body></html>", GURL("https://b.com"),
                      web_state());
  ...
  NSArray<NSString*>* disabled_scriptlets =
      ScriptletsForRules(@"b.*##+js(deamp)", @"https://b.com", resources_json,
