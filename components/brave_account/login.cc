...
void Login::Init(mojom::Service initiating_service,
  ...
  state_->SendStateOwnedRequest<endpoints::LoginInit>(
      std::move(request),
      base::BindOnce(&Login::OnInit, weak_factory_.GetWeakPtr(),
                     std::move(callback)));
}

void Login::Finalize(const std::string& encrypted_login_token,
  ...
  state_->SendStateOwnedRequest<endpoints::LoginFinalize>(
      std::move(request),
      base::BindOnce(&Login::OnFinalize, weak_factory_.GetWeakPtr(),
                     std::move(callback)));
}

void Login::OnInit(mojom::Authentication::LoginInitCallback callback,
  ...
  auto result =
      std::move(*response.body)
          ...
          // expected<[SuccessBody       ], LoginErrorPtr> ==>
          // expected<[LoginInitResultPtr], LoginErrorPtr>
          .and_then([&](auto success_body)
                        -> base::expected<mojom::LoginInitResultPtr,
                                          mojom::LoginErrorPtr> {
            ...
            return mojom::LoginInitResult::New(
...
void Login::OnFinalize(mojom::Authentication::LoginFinalizeCallback callback,
