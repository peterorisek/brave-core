...
void LoggedOutState::LoginInit(mojom::Service initiating_service,
                               const std::string& email,
                               const std::string& serialized_ke1,
                               LoginInitCallback callback) {
  login_.Init(initiating_service, email, serialized_ke1, std::move(callback));
}

void LoggedOutState::LoginFinalize(const std::string& encrypted_login_token,
                                   const std::string& client_mac,
                                   LoginFinalizeCallback callback) {
  login_.Finalize(encrypted_login_token, client_mac, std::move(callback));
}
