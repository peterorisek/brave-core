...
inline constexpr char kScriptletGlobalsScript[] =
    "const scriptletGlobals = (() => {"
    "const forwardedMapMethods = [\"has\", \"get\", \"set\"];"
    "const handler = {"
    "get(target, prop) { if (forwardedMapMethods.includes(prop)) { return "
    "Map.prototype[prop].bind(target) } return target.get(prop); },"
    "set(target, prop, value) { if (!forwardedMapMethods.includes(prop)) { "
